...
internal class dfChecksumUtil
{
    ...
    {
        checksum = crcTable[(int) ((IntPtr) ((checksum ^ ((byte) (value & 0xff))) & 0xff))] ^ (checksum >> 8);
        checksum = crcTable[(int) ((IntPtr) ((checksum ^ ((byte) ((value >> 8) & 0xff))) & 0xff))] ^ (checksum >> 8);
        checksum = crcTable[(int) ((IntPtr) ((checksum ^ ((byte) ((value >> 0x10) & 0xff))) & 0xff))] ^ (checksum >> 8);
        checksum = crcTable[(int) ((IntPtr) ((checksum ^ ((byte) ((value >> 0x18) & 0xff))) & 0xff))] ^ (checksum >> 8);
        return checksum;
    }
}
